...
//|  - GPT-Modell als DROPDOWN (Enum statt String)                    |
//|    -> 8 Modelle waehlbar im Input-Fenster                         |
//|  - LIVE-SCREENSHOT BRIDGE zum GOLDY Screenshot V 2.0              |
//|    (separater MT4 auf gleichem VPS)                               |
//|    -> Bei neuem Trade: Request-Datei in Common-Ordner schreiben   |
//|       Screenshot-EA macht Chart-Foto und sendet es an Telegram    |
//|       Goldy wartet auf Done-Datei (max. X Sek.)                   |
//|       Danach sendet Goldy sein Deko-Bild + Trade-Infos            |
//|    -> Reihenfolge in Telegram: 1. Live-Screenshot                 |
//|                                2. Deko-Bild (NewTrade.jpg)        |
//|                                3. Trade-Info-Text                 |
//|    -> Optional auch beim Schliessen eines Trades                  |
//|                                                                   |
//|  Alles andere (Trading, KI, Crop, Pyramiding, Umdrehen, Win/Loss) |
//|  bleibt unveraendert wie in V6.6                                  |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Alex"
#property version   "7.00"
#property description "GOLDY AI TRADER V 7.0 - mit Live-Screenshot Bridge"
#property strict
...
// V7.0: GPT-Modell-Auswahl (erscheint als Dropdown im Input-Fenster)
enum ENUM_GPT_MODEL
{
   GPT_4o,           // gpt-4o
...
//==== V7.0 NEU: BRIDGE ZUM SCREENSHOT-EA ===========================
input string   __Bridge_Header          = "==== SCREENSHOT-EA BRIDGE ====";
input ENUM_JN  Screenshot_Aktiv         = Ja;   // Bridge ein/aus
input ENUM_JN  Screenshot_Bei_Open      = Ja;   // bei neuem Trade
input ENUM_JN  Screenshot_Bei_Close     = Ja;   // beim Schliessen
...
input int      Bridge_Max_Warten_Sek    = 8;    // Timeout
input int      Bridge_Poll_ms           = 200;  // Pruefintervall
//===================================================================
...
int OnInit()
{
   ...
   if(Modus_Umdrehen == Ja)
      Print("UMDREHEN-MODUS AKTIV");
   else
      Print("Modus GETRENNT");
   if(Screenshot_Aktiv == Ja)
   {
      ...
      Print("BRIDGE AUS - kein Live-Screenshot");
...
//+------------------------------------------------------------------+
//| V7.0: SCREENSHOT-BRIDGE (Request + Warten auf Done)              |
//+------------------------------------------------------------------+
bool WarteAufScreenshot(string anlass)
{
   // Alte Done-Datei vorher entfernen, falls noch da
   if(FileIsExist(Bridge_Done_Datei, FILE_COMMON))
      FileDelete(Bridge_Done_Datei, FILE_COMMON);

   // Request-Datei in Common-Ordner schreiben
   ...
   // Timeout - trotzdem mit den Goldy-Nachrichten weitermachen
